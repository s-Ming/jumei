...
class Home extends Component {
  constructor(props) {
    ...
    this.state = {
      tab: 0,
      tablist: [
        { title: "今日10点上新", href: "tab-online", channel: "" },
        { title: "明日10点预告", href: "tab-preshow", channel: "" }
      ],
      homelist: []
    };
  }
  ...
  tabChange(arr) {
    let tabHtml = arr.map((item, index) => {
      /*console.log(item);*/
      return (
        <li key={index} onClick={this.getInfo.bind(this, index)}>
          {item.title}
        </li>
      );
    });
  ...
  render() {
    return (
      <div>
        <Hsearch />
        <Hnav />
        <ul className="tablist">{this.tabChange(this.state.tablist)}</ul>
        {this.state.homelist === [] ? (
          "111"
        ) : (
          <Hlist data={this.state.homelist} />
        )}
